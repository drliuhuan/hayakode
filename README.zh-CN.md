...
| 设置项 | 默认值 | 说明 |
| --- | --- | --- |
| 悬浮按钮显示阈值（设备像素） | `64` | 图片渲染短边（**CSS 像素 × 设备像素比**）≥ 该值才显示按钮；可调范围 32–200 |
...
> **为什么按设备像素判断？** 浏览器缩放（Ctrl +/-）只提升设备像素比、不改变 CSS 尺寸。按 CSS 判断会导致“用户放大页面后二维码已经可识别，按钮却仍不出现”。按设备像素判断可自动覆盖“放大后再识别”这一核心场景：40px 的图在 1 倍下不显示按钮，在 200% 缩放下（40 × 2 = 80 设备像素）就会显示。阈值只影响**是否显示悬浮按钮**，不影响识别区域的计算与裁剪。
...
- 显示阈值按**设备像素**判断：放大页面（Ctrl +）会提升设备像素比，达到阈值后按钮自动出现。
- 一维码默认关闭，可在设置中开启。
